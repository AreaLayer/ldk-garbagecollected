...
export class Payee extends CommonBase {
	...
	public static constructor_blinded(route_hints: TwoTuple_BlindedPayInfoBlindedPathZ[], features: Bolt12InvoiceFeatures): Payee {
		const ret: bigint = bindings.Payee_blinded(bindings.encodeUint64Array(route_hints.map(route_hints_conv_37 => CommonBase.get_ptr_of(route_hints_conv_37))), CommonBase.get_ptr_of(features));
		const ret_hu_conv: Payee = Payee.constr_from_ptr(ret);
	...
	public static constructor_clear(node_id: Uint8Array, route_hints: RouteHint[], features: Bolt11InvoiceFeatures, final_cltv_expiry_delta: number): Payee {
		const ret: bigint = bindings.Payee_clear(bindings.encodeUint8Array(bindings.check_arr_len(node_id, 33)), bindings.encodeUint64Array(route_hints.map(route_hints_conv_11 => CommonBase.get_ptr_of(route_hints_conv_11))), CommonBase.get_ptr_of(features), final_cltv_expiry_delta);
		const ret_hu_conv: Payee = Payee.constr_from_ptr(ret);
		CommonBase.add_ref_from(ret_hu_conv, ret_hu_conv);
