...
export class PeerDetails extends CommonBase {
	...
	public static constructor_new(counterparty_node_id_arg: Uint8Array, socket_address_arg: Option_SocketAddressZ, init_features_arg: InitFeatures, is_inbound_connection_arg: boolean): PeerDetails {
		const ret: bigint = bindings.PeerDetails_new(bindings.encodeUint8Array(bindings.check_arr_len(counterparty_node_id_arg, 33)), CommonBase.get_ptr_of(socket_address_arg), CommonBase.get_ptr_of(init_features_arg), is_inbound_connection_arg);
		const ret_hu_conv: PeerDetails = new PeerDetails(null, ret);
		CommonBase.add_ref_from(ret_hu_conv, ret_hu_conv);
		CommonBase.add_ref_from(ret_hu_conv, socket_address_arg);
		CommonBase.add_ref_from(ret_hu_conv, init_features_arg);
		return ret_hu_conv;
	}
