...
export class Record extends CommonBase {
	...
	public static constructor_new(level_arg: Level, peer_id_arg: Uint8Array|null, channel_id_arg: ChannelId|null, args_arg: string, module_path_arg: string, file_arg: string, line_arg: number, payment_hash_arg: Option_ThirtyTwoBytesZ): Record {
		const ret: bigint = bindings.Record_new(level_arg, bindings.encodeUint8Array(bindings.check_arr_len(peer_id_arg, 33)), channel_id_arg == null ? 0n : CommonBase.get_ptr_of(channel_id_arg), bindings.encodeString(args_arg), bindings.encodeString(module_path_arg), bindings.encodeString(file_arg), line_arg, CommonBase.get_ptr_of(payment_hash_arg));
