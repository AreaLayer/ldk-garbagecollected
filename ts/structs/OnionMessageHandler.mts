...
export class OnionMessageHandler extends CommonBase {
	...
	public static new_impl(arg: OnionMessageHandlerInterface): OnionMessageHandler {
		...
		let structImplementation = {
			get_and_clear_connections_needed (): number {
				...
				const result: number = bindings.encodeUint64Array(ret.map(ret_conv_40 => ret_conv_40.clone_ptr()));
				return result;
			},
	...
	public peer_connected(their_node_id: Uint8Array, init: Init, inbound: boolean): Result_NoneNoneZ {
		const ret: bigint = bindings.OnionMessageHandler_peer_connected(this.ptr, bindings.encodeUint8Array(bindings.check_arr_len(their_node_id, 33)), CommonBase.get_ptr_of(init), inbound);
