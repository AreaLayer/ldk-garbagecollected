...
export class AsyncPaymentsContext extends CommonBase {
	 ...
	 */
	public static constructor_outbound_payment(payment_id: Uint8Array, nonce: Nonce, hmac: Uint8Array): AsyncPaymentsContext {
		const ret: bigint = bindings.AsyncPaymentsContext_outbound_payment(bindings.encodeUint8Array(bindings.check_arr_len(payment_id, 32)), CommonBase.get_ptr_of(nonce), bindings.encodeUint8Array(bindings.check_arr_len(hmac, 32)));
		const ret_hu_conv: AsyncPaymentsContext = AsyncPaymentsContext.constr_from_ptr(ret);
...
export class AsyncPaymentsContext_OutboundPayment extends AsyncPaymentsContext {
	...
	public constructor(ptr: bigint) {
		...
		const nonce: bigint = bindings.LDKAsyncPaymentsContext_OutboundPayment_get_nonce(ptr);
		const nonce_hu_conv: Nonce = new Nonce(null, nonce);
			CommonBase.add_ref_from(nonce_hu_conv, this);
		this.nonce = nonce_hu_conv;
		const hmac: number = bindings.LDKAsyncPaymentsContext_OutboundPayment_get_hmac(ptr);
